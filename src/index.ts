...
import { registerBuiltins } from './registry/builtin.ts';

// Register built-in middlewares and presets
registerBuiltins();
...
try {
  ...
  if (error.message?.includes('Config file not found')) {
    ...
    console.error('Please create a chaos.yaml file in your project root or specify a path with --config <path>.');
  } else {
